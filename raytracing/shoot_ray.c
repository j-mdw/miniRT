    ...
    set_pov_plan(double *orient_vec, t_ray *ray_ptr)
{
    ...
    ray_ptr->vec_v[0], ray_ptr->vec_v[1], ray_ptr->vec_v[2], ray_ptr->vec_w[0], ray_ptr->vec_w[1], ray_ptr->vec_w[2]);
     printf("Dot products: u.v|%f|v.w|%f|u.w|%f|\n", dot_prod(ray_ptr->vec_u, ray_ptr->vec_v, 3), dot_prod(ray_ptr->vec_v, ray_ptr->vec_w, 3), dot_prod(ray_ptr->vec_u, ray_ptr->vec_w, 3));
*/}
    ...
    shoot_ray(t_param *p_ptr, t_ray *ray_ptr, int x, int y)
{
    ...
    if (obj_distance > 0.0)
    {
        ...
        {
            vec_substract(tmp_vec1, closest_surface->coord2, closest_surface->coord1, 3);
            vec_substract(tmp_vec2, closest_surface->coord3, closest_surface->coord2, 3);
